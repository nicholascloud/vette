  ...
  suite('Vette.gt()', function () {
    ...
    suite('inclusive', function () {
      ...
      test('should return a message when value is not greater than a given number', function (done) {
        $field.val(9);
        var expected = 'error';
        var rule = Vette.gt(10, true, expected);
        var actual = rule($field)
    ...
    suite('exclusive', function () {
      ...
      test('should return a message when value is equal to a given number', function (done) {
        $field.val(10);
        var expected = 'error';
        var rule = Vette.gt(10, false, expected);
        var actual = rule($field)
        assert.isString(actual);
        assert.equal(expected, actual);
        done();
      ...
      test('should return a message when value is less than a given number', function (done) {
        $field.val(9);
        var expected = 'error';
        var rule = Vette.gt(10, false, expected);
        var actual = rule($field)
